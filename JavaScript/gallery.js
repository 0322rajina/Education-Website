document.addEventListener('DOMContentLoaded', () => {
    ...
    function show(i, firstLoad = false) {
        ...
        yearChips.forEach(c => c.classList.toggle('active', Number(c.dataset.targetIndex) === index));
    }
    ...
    if(playBtn) playBtn.addEventListener('click', () => timer ? stop() : start());
    thumbs.forEach(t => t.addEventListener('click', () => { stop(); show(Number(t.dataset.index) || 0); }));
    yearChips.forEach(c => c.addEventListener('click', () => { stop(); show(Number(c.dataset.targetIndex) || 0); }));
    if(leftArrow) leftArrow.addEventListener('click', () => { stop(); show(index - 1); });
    if(rightArrow) rightArrow.addEventListener('click', () => { stop(); show(index + 1); });

    slides.forEach(s => {
        s.el.querySelectorAll('button, a').forEach(btn => btn.style.pointerEvents = 'auto');
    });
